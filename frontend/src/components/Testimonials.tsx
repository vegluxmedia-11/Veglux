...
import { getTestimonials, ReviewItem, VideoTestimonial } from "@/lib/services";
...
export default function Testimonials() {
  ...
  const [videos, setVideos] = useState<VideoTestimonial[]>(defaultVideos);
  ...
  useEffect(() => {
    async function loadTestimonials() {
      try {
        const { textReviews, videoReviews } = await getTestimonials();
        ...
        console.warn("Using default testimonial fallback", err);
      }
    }
    loadTestimonials();
  }, []);
  ...
  useEffect(() => {
    if (activeVideo) {
      ...
      const subtitlesCount = activeVideo.subtitles?.length || 1;
      progressTimer = setInterval(updateProgress, (subtitlesCount * interval) / 100);
      
      const subtitleTimer = setInterval(() => {
        setSubIdx((prev) => {
          if (activeVideo.subtitles && prev < activeVideo.subtitles.length - 1) {
            return prev + 1;
          } else {
    ...
    <section id="testimonials" className={styles.testimonials}>
      <div className={`${styles.container} container`}>
        ...
        <div className={styles.reviewsGrid}>
          {reviews.map((rev, idx) => (
            <div key={rev._id || idx} className={styles.reviewCard}>
        ...
        {videos.length > 0 && (
          <div>
            ...
            <div className={styles.videosGrid}>
              {videos.map((vid, idx) => (
                <div key={vid.id || vid._id || idx} className={styles.videoCard} onClick={() => setActiveVideo(vid)}>
          ...
          <div className={styles.videoModalContent} onClick={(e) => e.stopPropagation()}>
            ...
            <div className={styles.mockPlayer}>
              ...
              <div className={styles.mockSubtitles}>
                {activeVideo.subtitles && activeVideo.subtitles[subIdx]}
              </div>
